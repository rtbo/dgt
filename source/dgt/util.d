...
{
    enum isConstructible =
            !is(T == interface) && !__traits(isAbstractClass, T);
}


package template EmplaceSize(T)
        if (isConstructible!T)
{
...
mixin template SignalValueProperty(string __name, T, T defaultVal=T.init)
{
    ...
    mixin("public @property void "~__name~"(T value) {\n" ~
          ...
          "        "~__name~"_ = value;\n" ~
          "        "~onChangeSigName(__name)~"_.fire(value);\n" ~
          "    }\n" ~
          "}");
    mixin("public @property Signal!T "~onChangeSigName(__name)~"() {\n" ~
...
mixin template EventHandlerSignalMixin(string __name, HandlerT)
{
    ...
    mixin("public @property EventHandlerSignal!HandlerT "~__name~"() { return "~__name~"_; }");
}
...
string capitalizeFst(string input) in {
