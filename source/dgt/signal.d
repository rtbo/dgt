...
template isEventHandler(Iface) {
    static if (isSmi!Iface) {
        enum bool isEventHandler =
                smiParamsType!(Iface).length == 1 &&
...
abstract class SmiSignal (Iface)
        if (isSmi!Iface &&
            is(smiRetType!Iface == void))
{
    alias RetType = void;
...
final class FireableSmiSignal (Iface)
        if (isSmi!Iface &&
            is(smiRetType!Iface == void))
        : SmiSignal!(Iface)
{
...
unittest
{
    ...
    class C {
        ...
        void fireSig(int v) { s_.fire(v); }

        @property SmiSignal!I1 s() { return s_; }
    }
    auto c = new C;
    c.s += &f;
    c.fireSig(18);
...
abstract class EventHandlerSignal(HandlerT)
        if (isEventHandler!HandlerT)
{
    alias RetType = void;
...
final class FireableEventHandlerSignal(HandlerT)
        if (isEventHandler!HandlerT) :
